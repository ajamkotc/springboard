...
FROM Members
WHERE memid > 0
HAVING MAX(joindate) = MAX(joindate);
...
SELECT
	DISTINCT CONCAT_WS(" ", firstname, surname) AS fullname,
	f.name
FROM Members AS m
INNER JOIN Bookings
USING(memid)
...
SELECT
	...
	CASE WHEN m.memid = 0 THEN sub.guestcost
...
WHERE (sub.membercost * sub.slots > 30) OR (sub.guestcost * sub.slots > 30)
ORDER BY cost DESC;
